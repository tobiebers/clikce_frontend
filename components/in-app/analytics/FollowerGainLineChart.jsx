...
export default function Chart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:5000/follower-data');
        if (!response.ok) {
  ...
  return (
    <div className="chart-container" ref={containerRef}>
      <AreaChart
        width={500} // Verwendung der dynamisch festgelegten Breite
        height={300}
        data={data}
      ...
      >
        ...
        <Area type="monotone" dataKey="Follower" stroke="#8884d8" fillOpacity={1} fill="url(#colorGradient)" />
      </AreaChart>
    </div>
  );
}
